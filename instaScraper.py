...
class Scraper:
    def __init__(self, username, password, target_username):
        ...
        self.driver = webdriver.Chrome('chromedriver_linux64/chromedriver.exe', chrome_options=options) # I'm using linux. You can change it as per your OS.
        self.main_url = 'https://www.instagram.com'
        
        # check the internet connection and if the home page is fully loaded or not. 
        try:
            self.driver.get(self.main_url)
            WebDriverWait(self.driver, 30).until(EC.presence_of_element_located((By.XPATH, '//div[text()="Log In"]')))
        except TimeoutError:
    ...
    def load_fetch_posts(self):
        ...
        #     no_of_posts = no_of_posts.text.replace(',', '')
    ...
    def download_posts(self):
        ...
        for index, img in enumerate(image_list, start = 1):
            try:
                filename = 'image_' + str(count + index) + '.jpg'
